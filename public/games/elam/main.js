...
// AI data constants and pure logic are in elam-ai/local-ai.js (loaded via ElamLocalAI global).
const localAi = (typeof ElamLocalAI !== "undefined") ? ElamLocalAI : null;
const AI_PROFILES = localAi ? localAi.AI_PROFILES : [];
const COMPETITIVE_AI_PROFILE = localAi ? localAi.COMPETITIVE_AI_PROFILE : {};
const AI_DIFFICULTY_SETTINGS = localAi ? localAi.AI_DIFFICULTY_SETTINGS : {};
const AI_DIFFICULTY_WEIGHT_MODS = localAi ? localAi.AI_DIFFICULTY_WEIGHT_MODS : {};
...
function activeAiProfile(player) {
  if (!localAi) return player.aiProfile || AI_PROFILES[0];
  return localAi.activeAiProfile(player, aiDifficultyKey());
}

function buildAiContext() {
  return {
    board: state.board,
    flagZone: state.flagZone,
    flag: state.flag,
...
function aiAct(player) {
  ...
  const selectedCandidate = localAi.selectAction(player, aiDifficultyKey(), ctx, Math.random);
  if (!selectedCandidate) return false;
  const executed = executeCandidate(player, selectedCandidate);
  if (executed) {
    logDev(localAi.describeAiChoice(player, selectedCandidate, aiDifficultyKey()));
  }
  return executed;
}
